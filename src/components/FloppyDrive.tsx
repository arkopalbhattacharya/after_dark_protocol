...
export function FloppyDrive({ 
  ...
  return (
    <div className="fixed bottom-11 left-6 md:left-12 z-20 select-none flex items-end gap-3 pointer-events-auto">
      ...
      {/* CLASSIFIED CASSETTE DECK // THE LOST CASSETTES OF JENNIFER (ONLY VISIBLE ONCE CIPHER UNLOCKED) */}
      {isCassetteUnlocked && onOpenCassetteModal && (
        ...
        >
          {/* Cyberpunk Sky Blue Container */}
          <div className="transition-all duration-300 ease-out transform translate-y-[30px] group-hover:translate-y-[-6px] bg-gradient-to-b from-[#38bdf8] to-[#0284c7] text-[#020617] border-x-2 border-b-2 border-[#0369a1] px-2.5 pt-[15px] pb-1.5 rounded-b-xs shadow-[0_0_14px_rgba(56,189,248,0.5),inset_0_0_8px_rgba(255,255,255,0.4)] group-hover:shadow-[0_0_22px_rgba(56,189,248,0.9),inset_0_0_12px_rgba(255,255,255,0.6)] relative flex flex-col items-center overflow-visible">
            {/* Micro CRT Scanline Overlay */}
            <div className="absolute inset-0 pointer-events-none opacity-20 bg-[repeating-linear-gradient(0deg,rgba(0,0,0,0.6)_0px,rgba(0,0,0,0.6)_1px,transparent_1px,transparent_3px)]"></div>
            ...
            <div className="absolute left-0.5 top-1 bottom-1 flex flex-col justify-between pointer-events-none opacity-70">
              {Array.from({ length: 4 }).map((_, i) => (
                <span key={i} className="w-1.5 h-1.5 bg-[#0369a1] border border-[#38bdf8]/60 block shadow-[inset_0_0_2px_rgba(0,0,0,0.8)]"></span>
              ))}
            </div>
            <div className="absolute right-0.5 top-1 bottom-1 flex flex-col justify-between pointer-events-none opacity-70">
              {Array.from({ length: 4 }).map((_, i) => (
            ...
            {/* Inner Vintage Cassette Tape Graphic */}
            <div className="relative w-11 h-12 flex flex-col items-center justify-center p-0.5 z-10">
              <div className="w-9 h-7 bg-[#020b18] border border-[#38bdf8] rounded-xs flex items-center justify-between px-1 shadow-[0_0_6px_rgba(56,189,248,0.6)]">
                <span className="w-2.5 h-2.5 rounded-full border border-[#38bdf8] bg-black block"></span>
                <span className="w-2 h-1 bg-[#38bdf8]/50 block"></span>
                <span className="w-2.5 h-2.5 rounded-full border border-[#38bdf8] bg-black block"></span>
              </div>
              <span className="text-[8px] font-black text-white tracking-widest mt-1 font-mono uppercase">
                JENNY
              </span>
            </div>
          </div>
